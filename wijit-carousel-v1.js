...
export class WijitCarousel extends HTMLElement {
	...
	constructor() {
		...
		<div id="container">
			<div id="panel" part="panel">
				...
				<div id="front" part="flipper" class="hidden">
					<slot name="front"></slot>
				</div>
				<div id="back" part="flipper" class="hidden">
					<slot name="back"></slot>
				</div>
			</div>
	...
	initControls() {
		for (let i = 0; i < this.controls.length; i++) {
			...
			this.controls[i].addEventListener('click', evt => {
				// preventDefault() prevents inputs from being auto focused on click();
	...
	initFlip() {
		...
		this.front = this.front || this.shadow.querySelector('#front');
...
this.slider.classList.remove('autoslide');
	...
	initAutoSlide() {
		...
		this.slider.addEventListener('animationstart', (evt) => {
			...
			// click appropriate control on slide advance
			this.animatedControlInterval = setInterval (() => {
				counter++;
		...
		// Stop clicking controls when this.repeat is reached
		let iteration = 1;
	...
	slideEffect (evt, smooth = true) {
		// this.slider.classList.remove('autoslide');
		if (!evt.isTrusted && this.auto && this.effect === "slide") return;
	...
	set auto (value) {
		...
		if (this.#auto) {
			...
			this.slider.classList.remove('autoslide');
			clearInterval(this.animatedControlInterval);
		}
	}
